...
global el Display EyeData Params

if ini == 0
tmp = EyelinkInit(0);
end
el = EyelinkInitDefaults(Display.wPtr);

el.backgroundcolour = 255/2;
...
Eyelink('Command','screen_pixel_coords = %ld %ld %ld %ld', 0,0,Display.rect(3)-1, Display.rect(4)-1);
Eyelink('Message', 'DISPLAY_COORDS %ld %ld %ld %ld', 0,0,Display.rect(3)-1, Display.rect(4)-1);

Eyelink('Command', 'link_sample_data = LEFT,RIGHT,GAZE,AREA'); 
Eyelink('Command', 'file_sample_data = LEFT,RIGHT,GAZE,AREA');

if PracOn
    EyeData.edfFile=sprintf('%demo_LagPrac_%s.edf', Params.dir, Params.sub.info);% overwrite the edf each time. or it would full the hard drive of eyelink computer
else
    EyeData.edfFile=sprintf('%demo_Main_%s.edf', Params.dir, Params.sub.info);
end
...
WaitSetMouse(Display.xCenter,Display.yCenter,0); % set cursor and wait for it to take effect
